...
const
 DefaultIdentValue = $F3D8;
 DefaultAccessMode = IPC_CREAT or SHM_R or SHM_W;
 DefaultBlockSize = SizeOf(LongWord);

type
 TSharedMemoryRec = packed record
  shmid :Integer;
  MemLongInt :^LongWord;
  Usesful :Boolean;
end;
...
type
 TSharedMemoryConfig = packed record
  IdentValue :Longint;
  AccessMode :Longint;
  BlockSize :Longint;
end;
...
{$ifdef unix}
 ...
 procedure DefaultConfigForSharedMemory(var Config :TSharedMemoryConfig);
 begin
  Config.IdentValue := DefaultIdentValue;
  Config.AccessMode := DefaultAccessMode;
